...
namespace serial {
    ...
    int Serial::SerialImpl::waitforsent(uint32_t timeout, size_t * returned_size)
    {
        ...
        if(!GetOverlappedResult(fd_, &_wo, &w_len, FALSE))
        {
            ans = -2;
        }
    _final:
    ...
    int Serial::SerialImpl::waitforrecv(uint32_t timeout, size_t * returned_size)
    {
        ...
        if(!GetOverlappedResult(fd_, &_ro, &r_len, FALSE))
        {
            ans = -2;
        }
    ...
    size_t Serial::SerialImpl::write (const uint8_t *data, size_t length) {
        ...
        if (!::WriteFile(fd_, data, static_cast<DWORD>(length), &bytes_written, &writeCompletionOverlapped)) {
            ...
            if(systemErrorCode != ERROR_IO_PENDING&&systemErrorCode!= ERROR_SUCCESS&& systemErrorCode != ERROR_MORE_DATA )
               return 0;
		}
