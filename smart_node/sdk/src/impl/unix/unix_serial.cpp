...
# endif
    ...
    static inline struct pollfd make_pollfd(int fd, short events)
    {
    ...
    */
    int safe_poll(struct pollfd *fds, nfds_t nfds, int timeout)
    {
        if (timeout <0) {
            ...
            int ret  = qt_ppoll(fds, nfds, NULL);
            return ret;
        }
    ...
    static inline int qt_poll_msecs(struct pollfd *fds, nfds_t nfds, int timeout)
    {
        ...
        return safe_poll(fds, nfds, timeout);
    }
    ...
    static inline void set_common_props(termios *tio) {
        ...
        //原始模式是一种特殊的非规范模式， 在原始模式下， 所有的输入数据以字节单位被处理。在这个模式下， 终端是不可回显的，
        //而且所有特定的终端输入/输出控制处理不可用。 通过调用cfmakeraw()函数可以将终端设置为原始模式。
#endif
    ...
    flags |= O_CLOEXEC;
        ...
        fd_ = ::open (port_.c_str(),  flags, 0777);
        static int count = 0;
		if (fd_ == -1) {
			switch (errno) {
			case EINTR:
    ...
    int Serial::SerialImpl::waitforsent(uint32_t timeout, size_t * returned_size)
    {
        return -2;

    }

    int Serial::SerialImpl::waitforrecv(uint32_t timeout, size_t * returned_size)
    {
        return waitfordata(65535, timeout, returned_size);
	...
	bool Serial::SerialImpl::setBreak (bool level) {
        ...
        if(::ioctl(fd_, level?TIOCSBRK:TIOCCBRK) == -1){
            return false;
        }
		return true;
	}
	...
	bool Serial::SerialImpl::setRTS (bool level){
        ...
        if (::ioctl(fd_, level ? TIOCMBIS : TIOCMBIC, &command) == -1) {
            return false;
        }
        return true;
	}
